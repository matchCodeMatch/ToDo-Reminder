    ...
    async function loadTask() {
      ...
      try {
        ...
        if (foundTask) {
          ...
          setRepeatRule(foundTask.repeat_rule);
          setRepeatDays(foundTask.repeat_days);
          setAlarmEnabled(foundTask.alarm_enabled);

          const [year, month, day] = foundTask.due_date.split('-').map(Number);
          setDueDate(new Date(year, month - 1, day));

          const [hours, minutes] = foundTask.due_time.split(':').map(Number);
